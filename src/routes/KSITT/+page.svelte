<h1>Main Page [BETA]</h1>

<p>
	This wiki is dedicated to recording expert level knowledge of Super Mario 64 RomHacks.
	The purpose of this wiki is to allow people to find information that isn't easily available on other websites or by a quick Google search.
	If you want to contribute, feel free to join the <a href="https://discord.gg/dBA3dcRaqS">Wiki Discord server</a>.
	Big shoutout to the Ukikipedia devs for making the main game wiki, it was really useful to make the romhacks speedrunning one.
	Enjoy!
</p>

<div id="audiences" class="mainpage_row">
	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Speedrun Guide (RTA)</span>
		</h3>

		<div id="mainpage-admins" class="items">
			&#8226; <a href="KSITT/Guide">Speedrunning (RTA) Start Page [WIP]</a><br>
			&#8226; <a href="KSITT/IL">Individual Stage Guides [WIP]</a><br>
			&#8226; <a href="KSITT/40_Burger">40 Burger Guide (v1.3.1) [WIP]</a><br>
			&#8226; <a href="KSITT/30_Burger">30 Burger Guide (v1.3.1) [WIP]</a><br>
			&#8226; <a href="KSITT/0_Burger">0 Burger (v1.2) Guide</a><br>
		</div>
	</div>

	<div class="mainpage_box">
		<h3>Glitches</h3>

		<div id="mainpage-users" class="items">
            &#8226; Backwards Long Jump<br>
            &#8226; Wall Clipping<br>
            &#8226; Glitchy Wall Kick<br>
            &#8226; Hyperjump<br>
		</div>
	</div>

	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Courses</span>
		</h3>

		<div id="mainpage-devs" class="items">
            &#8226; List of Courses<br>
		</div>

		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Objects</span>
		</h3>

		<div id="mainpage-admins" class="items">
            &#8226; Objects<br>
            &#8226; Object Slots<br>
		</div>
	</div>

	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Shoutouts</span>
		</h3>

		<div id="mainpage-admins" class="items">
            &#8226; Chiara*<br>
            &#8226; Dackage<br>
	    &#8226; Miphasidon<br>
	    &#8226; katze789<br>
	    &#8226; TriforceTK<br>
	    &#8226; wRadion<br>
		</div>
	</div>
</div>

<div id="audiences" class="mainpage_row">
	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Tool-Assisted Speedruns (TAS)</span>
		</h3>

		<div id="mainpage-devs" class="items">
			&#8226; No TAS yet<br>
		</div>
	</div>

	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Resources</span>
		</h3>

		<div id="mainpage-users" class="items">
            &#8226; Emulators<br>
            &#8226; Patch<br>
            &#8226; Leaderboard<br>
            &#8226; List of TAS World Records<br>
		</div>
	</div>

	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Challenges</span>
		</h3>

		<div id="mainpage-admins" class="items">
            &#8226; Challenges<br>
            &#8226; A Button Challenge (shoutout to sapling)<br>
            &#8226; B Button Challenge<br>
            &#8226; RTABC  (Real Time A Button Challenge)<br>
            &#8226; CCC Challenge [WIP]<br>
		</div>
	</div>
</div>

<div id="audiences" class="mainpage_row">
	<div class="mainpage_box">
		<h3>
			<span class="header_icon" aria-hidden="true" role="presentation"></span>
			<span>Main Game Wiki</span>
		</h3>

		<div id="mainpage-devs" class="items">
            &#8226; <a href="https://ukikipedia.net">Ukikipedia</a>
		</div>
	</div>
</div>

<style>
	.mainpage_row {
		display: flex;
		flex-direction: row;
		flex-flow: row wrap;
		justify-content: space-between;
		padding: 0;
		margin: 0 -5px 0 -5px;
	}

	.mainpage_box {
		flex: 1;
		margin: 5px;
		font-size: 90%;
		border: 1px solid #CCC;
		padding: 0 10px 10px 10px;
		box-shadow: 0 2px 2px rgba(0,0,0,0.1);
		font-family: Arial, Helvetica, sans-serif;
		line-height: 1.5;
	}

	.mainpage_box h3 {
		/* Reset padding so is equal across skins */
		padding: 0;
		margin: 0.5em 0;
	}

	.mainpage_box h3 .header_icon {
		float: left;
		margin-right: 5px;
	}

	.mainpage_box .items {
		border-top: 1px solid #CCC;
		padding-top: 0.5em;
	}
</style>